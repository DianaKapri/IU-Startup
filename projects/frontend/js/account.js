...
/* ═══ Auth check ═══ */
if (!spRequireAuth()) return;
var user = spGetCurrentUser();

/* ═══ Fill nav header ═══ */
function refreshHeader() {
  user = spGetCurrentUser();
  ...
  if (accSchool)  accSchool.textContent  = user.school;
  if (accAvatar)  accAvatar.textContent  = (user.name || 'У').charAt(0).toUpperCase();
  if (dashTitle)  dashTitle.textContent  = user.school || 'Личный кабинет';
  if (accWelcome) accWelcome.textContent = 'Добро пожаловать, ' + firstName + '. Загрузите файл расписания — аудит займёт секунды.';
  if (navBuyBtn)  navBuyBtn.style.display = user.plan === 'paid' ? 'none' : '';
}
refreshHeader();
...
if (profileForm) {
  profileForm.addEventListener('submit', function (e) {
    ...
    var currentUser = spGetCurrentUser();
    var users = JSON.parse(localStorage.getItem('shkolaplan_users') || '[]');
    var emailConflict = users.some(function (u) {
      return u.email === emailVal && u.id !== currentUser.id;
    });
    if (emailConflict) { profileErr('profileEmailErr', 'Этот email уже занят'); return; }

    users = users.map(function (u) {
      if (u.id !== currentUser.id) return u;
      var updated = Object.assign({}, u, { name: nameVal, school: schoolVal, email: emailVal });
      if (passVal) updated.password = btoa(unescape(encodeURIComponent(passVal)));
      return updated;
    });
    localStorage.setItem('shkolaplan_users', JSON.stringify(users));

    var updatedSession = Object.assign({}, currentUser, { name: nameVal, school: schoolVal, email: emailVal });
    localStorage.setItem('shkolaplan_current_user', JSON.stringify(updatedSession));
...
function showResults(sch, cg) {
  ...
  if (auditEl) {
    ...
    classes.forEach(function (cl) {
      ...
      ringEl.className = 'acc-score-card';
      scoreRing(audit.score, ringEl.appendChild(document.createElement('div')));
