...
var wizData={
  schoolName:'Школа №42, г. Москва', shifts:1, days:5, mode:'fast',
  teachers:[
...
document.addEventListener('click',function(e){
  ...
  var rawVal=opt.getAttribute('data-val');
  if(toggle.id==='wizShiftsToggle')wizData.shifts=+rawVal;
  if(toggle.id==='wizDaysToggle')wizData.days=+rawVal;
  if(toggle.id==='wizModeToggle')wizData.mode=rawVal;
});
...
function wizOpenSchedule(){
  ...
  spAuthHeaders().then(function(authH){
    ...
    return fetch('/api/generate',{
      method:'POST',
      headers:headers,
      body:JSON.stringify({classes:classes,curriculum:curriculum,weekDays:days===6?6:5,mode:mode}),
    });
  })
    .then(function(r){return r.json();})
    .then(function(resp){
